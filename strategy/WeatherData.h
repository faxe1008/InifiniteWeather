...
public:
  ...
  enum class obscurationtype {
    ...
    Sand = 6,
    /** Water particles suspended in the air; low-ish visibility; temperature is
       near dewpoint */
    Mist = 7,
    /** This is SPECIAL in the sense that the thing raining down is doing the
       obscuration */
    Precipitation = 8,
    Length
  };
  ...
  enum class precipitationtype {
    ...
    SnowGrains = 8,
    /** Needles; columns or plates of ice. Sometimes described as "diamond
       dust". In very cold regions */
    IceCrystals = 9,
  ...
  /**
   * These are special events that can "enhance" the "experience" of existing
   * weather events
   */
  ...
  public:
     ...
     *
     * If there's a newer event of the same type then it overrides this one,
   ...
   *
   * As direction can fluctuate wildly and some watch faces might wish to
   * display it nicely, we're following the aerospace industry weather report
   * option of specifying a range.
   */
   ...
   * this allows watch face or watchapp makers to generate accurate alerts and
   * graphics
   *
  ...
  public:
     ...
     * For pollen use the genus, e.g. "Betula" for birch or "Alternaria" for
     * that mold's spores
     */
    IString polluter;
    /**
     ...
     *
     * Note: ppb and ppm to concentration should be calculated on the companion,
     * using the correct formula (taking into account temperature and air
     * pressure)
     *
     * Note2: The amount is off by times 100, for two decimal places of
     * precision. E.g. 54.32µg/m³ is 5432
     *
     */
    uint32_t amount;
  };
